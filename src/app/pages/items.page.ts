...
@Component({
  ...
  <ion-modal [isOpen]="open" (didDismiss)="open=false"><ng-template><ion-header><ion-toolbar><div class="modal-title"><ion-buttons><ion-button (click)="open=false">Cancelar</ion-button></ion-buttons><b>Nuevo objeto</b><span></span></div></ion-toolbar></ion-header>
    <ion-content><div class="form-sheet">
      ...
      <ion-button class="primary-action" expand="block" (click)="save()" [disabled]="!name.trim()">Guardar objeto</ion-button>
    </div></ion-content>
...
export class ItemsPage{
  d=inject(DataService);rows=signal<any[]>([]);locations=signal<Location[]>([]);boxes=signal<Box[]>([]);open=false;name='';description='';category='Otros';owner:Owner='Compartido';location:string|null=null;box:string|null=null;file?:File;
  constructor(){addIcons({add,trashOutline,cubeOutline,cameraOutline,personOutline,peopleOutline});this.load();}
  ...
  async save(){await this.d.addItem({name:this.name,description:this.description,category:this.category,owner:this.owner,location_id:this.box?null:this.location,box_id:this.box},this.file);this.open=false;this.name='';this.description='';this.file=undefined;await this.load();}
  async del(id:string){await this.d.remove('items',id);await this.load();}
}
