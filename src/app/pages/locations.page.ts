...
@Component({
  ...
  template: `
    ...
    <ion-content><main class="page-shell">
      ...
      @if(rows().length){<div class="fi-grid locations">
        @for(x of rows();track x.id){<article class="fi-card location-card">
          ...
          <ion-button class="delete danger-quiet" fill="clear" (click)="del(x.id)" aria-label="Enviar a la papelera"><ion-icon name="trash-outline"/></ion-button>
        </article>}
    ...
    <ion-modal [isOpen]="open" (didDismiss)="open=false"><ng-template>
      <ion-header><ion-toolbar><div class="modal-title"><ion-buttons><ion-button (click)="open=false">Cancelar</ion-button></ion-buttons><b>Nueva ubicación</b><span></span></div></ion-toolbar></ion-header>
      ...
      <ion-button class="primary-action" expand="block" (click)="save()" [disabled]="!name.trim()">Guardar ubicación</ion-button></div></ion-content>
  ...
  styles: [`
    ...
    .location-card{position:relative;display:grid;grid-template-columns:58px 1fr auto;align-items:center;gap:14px;padding:16px}.location-icon{width:58px;height:58px;display:grid;place-items:center;border-radius:18px;background:var(--fi-primary-soft);color:var(--fi-primary);font-size:26px}.location-card span{color:var(--fi-primary);font-size:.68rem;font-weight:800;text-transform:uppercase}.location-card h2{margin:3px 0;font-size:1rem}.location-card p{margin:0;color:var(--fi-muted);font-size:.76rem}.chevron{color:var(--fi-muted)}.delete{position:absolute;right:3px;top:1px;opacity:0}.location-card:hover .delete{opacity:1}.location-card:hover .chevron{opacity:0}.modal-title{height:58px;display:grid;grid-template-columns:1fr auto 1fr;align-items:center}.modal-title span{display:block}
  `]
})
export class LocationsPage {
  d=inject(DataService); rows=signal<Location[]>([]); open=false; name=''; type='Habitación'; parent:string|null=null;
  types=['Casa','Habitación','Armario','Estantería','Cajón','Trastero','Garaje','Personalizada'];
  constructor(){addIcons({add,trashOutline,homeOutline,locationOutline,bedOutline,businessOutline,fileTrayStackedOutline,chevronForwardOutline});this.load();}
  ...
  async save(){await this.d.addLocation({name:this.name,type:this.type,parent_id:this.parent});this.open=false;this.name='';this.type='Habitación';this.parent=null;await this.load();}
  async del(id:string){await this.d.remove('locations',id);await this.load();}
}
