...
import { add, archiveOutline, bedOutline, businessOutline, chevronBackOutline, chevronForwardOutline, createOutline, cubeOutline, fileTrayStackedOutline, homeOutline, locationOutline, peopleOutline, personOutline, trashOutline } from 'ionicons/icons';
  ...
  template: `
    ...
    <ion-content><main class="page-shell">
      ...
      @if(localItems().length){<div class="section-title"><h2>Objetos aquí</h2><span>{{localItems().length}}</span></div><div class="fi-grid">
        @for(x of localItems();track x.id){<a class="fi-card item-card editable-card" [href]="'#/objetos?editar=' + x.id">
    ...
    <ion-modal [isOpen]="open" (didDismiss)="close()"><ng-template>
      ...
      <div class="form-section"><h3>Ubicación</h3><ion-select label="Dentro de" labelPlacement="stacked" [(ngModel)]="parent"><ion-select-option [value]="null">Nivel principal</ion-select-option>@for(x of parentOptions();track x.id){<ion-select-option [value]="x.id">{{pathFor(x)}}</ion-select-option>}</ion-select></div>
...
export class LocationsPage {
  ...
  constructor(){addIcons({add,trashOutline,homeOutline,locationOutline,bedOutline,businessOutline,fileTrayStackedOutline,chevronForwardOutline,chevronBackOutline,createOutline,cubeOutline,archiveOutline,personOutline,peopleOutline});this.route.queryParamMap.subscribe(params=>{this.currentId.set(params.get('parent'));this.parent=this.currentId();if(params.get('nuevo')==='1')this.newLocation();this.load();});}
