...
@Component({
  ...
  <ion-modal [isOpen]="open" (didDismiss)="open=false"><ng-template><ion-header><ion-toolbar><div class="modal-title"><ion-buttons><ion-button type="button" (click)="open=false">Cancelar</ion-button></ion-buttons><b>Nueva caja</b><span></span></div></ion-toolbar></ion-header>
    <ion-content><div class="form-sheet">
      ...
      <ion-button type="button" class="primary-action" expand="block" (click)="save()" [disabled]="busy() || !name.trim()">{{ busy() ? 'Guardando...' : 'Guardar caja' }}</ion-button>
    </div></ion-content>
...
export class BoxesPage{
  d=inject(DataService);private route=inject(ActivatedRoute);rows=signal<any[]>([]);locations=signal<Location[]>([]);busy=signal(false);error=signal('');open=false;name='';description='';category='Otros';owner:Owner='Compartido';location:string|null=null;parentBox:string|null=null;file?:File;
  constructor(){addIcons({add,trashOutline,archiveOutline,cameraOutline,personOutline,peopleOutline});this.route.queryParamMap.subscribe(params=>{this.open=params.get('nuevo')==='1';});this.load();}
  ...
  async save(){this.busy.set(true);this.error.set('');try{await this.d.addBox({name:this.name,description:this.description,category:this.category,owner:this.owner,location_id:this.location,parent_box_id:this.parentBox},this.file);this.open=false;this.name='';this.description='';this.file=undefined;await this.load();}catch(e){this.error.set((e as Error).message||'No se pudo guardar la caja.');}finally{this.busy.set(false);}}
